...
export default function BulletinPage() {
    ...
    const payrollData = useMemo(() => {
        ...
        if (profile.hireDate) {
            try {
                ...
                if (seniorityYears >= 2) {
                    ...
                    seniorityBonus = seniorityTiers * (baseSalary * 0.04); // 4% for every 2 years
                }
            } catch (e) {
        ...
        if (profile.hireDate) {
            ...
            // Only count days from hire date to today within the cycle
            const cycleWorkDays = eachDayOfInterval({ start: max([cycleStart, hireDate]), end: min([cycleEnd, new Date()]) })
